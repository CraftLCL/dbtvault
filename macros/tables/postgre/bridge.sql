...
WITH as_of AS (
     ...
     WHERE a.AS_OF_DATE <= NOW() at time zone 'Asia/Shanghai'
),
...
bridge AS (
    ...
    FROM candidate_rows AS c
        {%- for bridge_step in bridge_walk.keys() -%}
            {%- set bridge_end_date = dbtvault.escape_column_names(bridge_walk[bridge_step]['bridge_end_date']) -%}
            {%- if loop.first %}
    WHERE c.{{ bridge_end_date }} :: TIMESTAMP = '{{ max_datetime }}' :: TIMESTAMP
            {%- else %}
        AND c.{{ bridge_end_date }} :: TIMESTAMP = '{{ max_datetime }}' :: TIMESTAMP
            {%- endif -%}
        {%- endfor %}
)
