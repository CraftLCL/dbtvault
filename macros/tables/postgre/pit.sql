...
backfill AS (
    ...
    {%- for sat_name in satellites -%}
        ...
        '{{ ghost_pk }}' ::bytea   AS {{ dbtvault.escape_column_names( sat_name ~ '_' ~ sat_pk_name ) }},
...
new_rows AS (
    ...
    {%- for sat_name in satellites -%}
        ...
        {%- set sat_pk = dbtvault.escape_column_names(satellites[sat_name]['pk'][sat_pk_name]) -%}
        {%- set sat_ldts = dbtvault.escape_column_names(satellites[sat_name]['ldts'][sat_ldts_name]) %}
        COALESCE(MAX({{ dbtvault.escape_column_names( sat_name | lower ~ '_src' ) }}.{{ sat_pk }}), '{{ ghost_pk }}' ::bytea ) AS {{ dbtvault.escape_column_names( sat_name | upper ~ '_' ~ sat_pk_name | upper ) }},
